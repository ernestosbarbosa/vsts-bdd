...
class Common {
    ...
    static page(page) {
        return safeLoad(readFileSync(resolve("./test/elements/" + page + ".yml"), 'utf8'), { schema: JSON_SCHEMA });
    }

    static element(page, el) {
        switch (this.page(page)[page][el]['tipo_busca']) {
            ...
            case TIPO_BUSCA.LINKTEXT:
                return element(by.linkText(this.page(page)[page][el]['value']))
            default:
                break;
        }
    }
    static waitElement(page, el) {
        switch (this.page(page)[page][el]['tipo_busca']) {
            case TIPO_BUSCA.CSS:
            ...
            case TIPO_BUSCA.LINKTEXT:
                return browser.wait(ExpectedConditions.presenceOf(element(by.linkText(this.page(page)[page][el]['value'])))).then(result => {
                    return result;
                })
            default:
                break;
